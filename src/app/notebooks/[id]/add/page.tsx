...
export default function AddErrorPage() {
    ...
    const handleClearClipboard = async () => {
        try {
            ...
            frontendLogger.info('[AddError]', 'Clipboard cleared successfully');
            // Clear local state
            setClipboardImage(null);
            setShowClipboardDialog(false);
        } catch (error) {
            frontendLogger.error('[AddError]', 'Failed to clear clipboard', {
                error: error instanceof Error ? error.message : String(error)
            });
            // Even if API fails, close dialog and clear local state
            setClipboardImage(null);
            setShowClipboardDialog(false);
        }
    };
            ...
            <Dialog open={showClipboardDialog} onOpenChange={setShowClipboardDialog}>
                <DialogContent>
                    ...
                    <DialogFooter className="flex-col sm:flex-row gap-2">
                        <Button variant="ghost" onClick={handleClearClipboard} className="text-sm">
                            {t.app?.clearClipboard || "清空系统剪贴板内容"}
                        </Button>
                        <div className="flex gap-2 w-full sm:w-auto">
                            <Button variant="outline" onClick={handleRejectClipboardImage} className="flex-1 sm:flex-none">
                                {t.common?.cancel || "取消"}
                            </Button>
                            <Button onClick={handleUseClipboardImage} className="flex-1 sm:flex-none">
                                {t.common?.confirm || "确认"}
                            </Button>
                        </div>
                    </DialogFooter>
